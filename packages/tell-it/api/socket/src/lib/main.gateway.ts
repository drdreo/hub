...
export class MainGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    onJoinRoom(
        ...
        @MessageBody() { userID, roomName, userName }: UserJoinMessage
    ): WsResponse<ServerJoined> {
                ...
                if (e instanceof RoomStartedError || e instanceof RoomFullError) {
                    if (room && room.getConfig().spectatorsAllowed) {
                        ...
                        return {
                            ...
                            data: { room: sanitizedRoom }
                        };
                    } else {
                        this.logger.debug(
                            "Couldnt create or join room, but spectating is not allowed!"
                        );
    ...
    onJoinSpectator(
        @ConnectedSocket() socket: Socket,
        @MessageBody() { roomName }: UserSpectatorJoinMessage
    ): WsResponse<ServerSpectatorJoined> {
    ...
    private handleUserDisconnect(userID: string | undefined, room: string | undefined) {
        ...
        if (userID && this.roomService.userExists(userID)) {
            ...
            if (room) {
                ...
                this.logger.error(
                    `User[${userID}] disconnected or left, but room[${room}] no longer exists!`
                );
            }
        }
    }
    ...
    private async sendUserTextIndividually(roomName: string) {
        ...
        for (const user of room.getUsersPreview()) {
            ...
            if (socketId && !user.disconnected && room.isUserOwner(user.id)) {
                ...
                this.sendTo(
                    socketId,
                    ServerEvent.StoryUpdate,
                    newStoryData as ServerStoryUpdate | undefined
                );
            }
        }
    }
