...
import { defaultStats, extractPlayerStats, mergeStats } from "./src/app/game/game.utils";
import { FormattedGame, Game } from "./src/app/game/Game";
...
(async function main() {
    ...
    const games = allGames.filter(
        game =>
            game.data.startedAt &&
            game.data.startedAt._seconds * 1000 > 1573344000 &&
            game.data.startedAt._seconds * 1000 < new Date().getTime()
    );
    ...
    const lixStats = defaultStats;
    const floStats = defaultStats;
